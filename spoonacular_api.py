...
class Recipe:
    ...
    def __init__(self, _idx=None, _name='', _img_url='', _ingredients=list(), _missing=list(), _time=(0, 0, 0),
                 _instructions='', \
...
class SpoonacularAPI:
    ...
    def get_recipe_from_ingredients(self):
        query_str = "{}/recipes/findByIngredients?fillIngredients=true&ingredients={}&limitLicense=false&number={}&ranking=2".format(
        ...
        for trecipe_data in req_response:
            ...
            for ttmissing in trecipe_data.get('missedIngredients', []):
                missing.append(Ingredient(_idx=ttmissing['id'], _name=ttmissing['name'], _quantity=ttmissing['amount'],
                                          _units=ttmissing['unitShort']))

            used_ingredients = []
            for ttingredient in trecipe_data.get('usedIngredients', []):
                used_ingredients.append(
                    Ingredient(_idx=ttingredient['id'], _name=ttingredient['name'], _quantity=ttingredient['amount'],
                               _units=ttingredient['unitShort']))

            trecipe = Recipe(_idx=trecipe_data['id'], _name=trecipe_data['title'], _img_url=trecipe_data['image'],
                             _ingredients=used_ingredients + missing, _missing=missing)
    ...
    def update_recipe_data(self):
        ...
        for tidx, trecipe in enumerate(self.recipes):
            ...
            trecipe.time = tuple(
                [int(trecipe_data.get('readyInMinutes', 0)), int(trecipe_data.get('preparationMinutes', 0)),
                 int(trecipe_data.get('cookingMinutes', 0))])
            trecipe.servings = int(trecipe_data.get('servings', 1))
            trecipe.meal_type = set(
                [meal_vocab.get(tmeal, 'lunch') for tmeal in trecipe_data.get('dishTypes', ['lunch'])])
    ...
    def recipes_by_time(self, time_limit=Recipe.def_total_time, recipe_list=None):
        ...
        return sorted([rec for rec in recipe_list if sum(rec.time) <= time_limit], key=lambda rec: sum(rec.time),
                      reverse=False)
    ...
    def recipes_by_cost(self, cost_limit=Recipe.def_cost, recipe_list=None):
        ...
        return sorted([rec for rec in recipe_list if rec.cost <= cost_limit], key=lambda rec: sum(rec.price),
                      reverse=False)
